...
export class PersonajesComponent  {
  ...
  constructor( private apiPersonajes: AppRestService, 
    private router : Router ) {
    this.apiPersonajes.getDataPersonajes()
  ...
  setNameUrlPersonajes(){
    for ( let i = 0; i < this.listaPersonajes.length; i++ ) {
      ...
      for (let j = 0; j < urlCapitulos.length; j++) {
        ...
        this.apiPersonajes.getUrlListadoPersonajes(urli)
  ...
  setUrlSearchPersonajes( termino: string ){
    this.apiPersonajes.getFilterCapitulos( termino )
      .subscribe( ( data: any ) => {
